	...
	<div>
		<div class="home_box">
			...
			<div class="recomList_view">
				<div class="recomList_title">电影信息推荐</div>
				<div class="recommend_list_one">
					<div class="recommend_item animation_box" v-for="(item,index) in dianyingxinxiRecomList" :key="index" @click="detailClick('dianyingxinxi',item.id)">
						<div class="recommend_img_box">
							<img class="recommend_img" v-if="isHttp(item.haibao)" :src="item.haibao.split(',')[0]" alt="">
							<img class="recommend_img" v-else :src="item.haibao?$config.url + item.haibao.split(',')[0]:''" alt="">
						</div>
				...
				<div class="recommend_more_view" @click="moreClick('dianyingxinxi')">
					<span class="recommend_more_text">全部</span>
				</div>
			</div>
			<!-- 电影院信息首页展示 -->
			<div class="homeList_view">
				<div class="homeList_title">电影院信息展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in dianyingyuanxinxiHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
						...
						<div class="homeList_content">
							<div class="homeList_title1">
								{{item.dianyingyuanmingcheng}}
							</div>
							<div class="homeList_title2">
								联系方式：{{item.lianxifangshi}}
							</div>
						</div>
					</div>
				</div>
				<div class="homeList_more_view" @click="moreClick('dianyingyuanxinxi')">
					<span class="homeList_more_text">全部</span>
				</div>
			</div>
			<!-- 电影资讯 -->
			<div class="newsList_view">
				<div class="newsList_title">电影资讯</div>
...
<script setup>
	...
	//电影信息推荐
	const dianyingxinxiRecomList = ref([])
	const getdianyingxinxiRecomList = () => {
		let autoSortUrl = 'dianyingxinxi/autoSort'
		...
		context?.$http({
			...
			dianyingxinxiRecomList.value = res.data.data.list
		})
	}
	//电影院信息首页展示
	const dianyingyuanxinxiHomeList = ref([])
	const getdianyingyuanxinxiHomeList = () => {
		context?.$http({
			url: 'dianyingyuanxinxi/list',
			method: 'get',
			params: {
				page: 1,
				limit: 6
			...
			dianyingyuanxinxiHomeList.value = res.data.data.list
		})
	}
	//电影资讯弹窗
	...
	const init = () => {
		//电影信息推荐
		getdianyingxinxiRecomList()
		//电影院信息首页展示
		getdianyingyuanxinxiHomeList()
		//电影资讯
		getNewsList()
	}
	init()
