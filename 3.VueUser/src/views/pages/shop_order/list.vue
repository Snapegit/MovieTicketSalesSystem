		...
		<el-table v-loading="listLoading" border :stripe='true' @selection-change="handleSelectionChange" ref="table"
			...
			<el-table-column label="操作" :resizable='true' align="left" header-align="left" width="200px">
				<template #default="scope">
					...
					<el-button class="QRcode_btn" v-if="scope.row.status=='未支付'" type="primary" @click="QRcodePayClick(scope.row)">
						二维码支付
					</el-button>
					<el-button class="cancel_btn" v-if="scope.row.status=='未支付'" type="danger" @click="cancelClick(scope.row)">
						取消
					</el-button>
					<el-button class="exchange_btn" v-if="scope.row.status=='已支付'" type="primary" @click="exchange(scope.row)">
						兑换
					</el-button>
...
<script setup>
	...
	const QRcodePaySuccess = () => {
		ElMessageBox.confirm(`是否确定已支付该订单？`, '提示', {
			confirmButtonText: '是',
			cancelButtonText: '否',
			type: 'warning',
		}).then(() => {
				...
				tr {
					...
					td {
						...
						.cell {
							...
							// 二维码支付
							.QRcode_btn {
								border: 1px solid rgba(48,156,4,.3);
								cursor: pointer;
								border-radius: 4px;
								padding: 0 10px;
								margin: 0 5px 6px 0;
								outline: none;
								color: rgba(48,156,4,1);
								background: none;
								width: auto;
								font-size: 13px;
								height: 24px;
