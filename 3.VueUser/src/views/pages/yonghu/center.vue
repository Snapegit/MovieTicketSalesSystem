			...
			<div class="usersBox" v-if="tabIndex=='center'">
				<el-form class="usersForm" ref="userFormRef" :model="userForm" label-width="120px" :rules="rules">
					<el-row>
						...
						<el-col :span="8">
							<el-form-item prop="vip" label="是否会员">
								<div class="vip_item">
									<el-input class="vip_inp" v-model="userForm.vip" placeholder="是否会员" readonly></el-input>
									<el-button class="vip_btn" @click="vipClick">会员购买</el-button>
								</div>
							</el-form-item>
						</el-col>
					</el-row>
...
<el-dialog v-model="vipVisible" :title="'会员购买'" width="50%" destroy-on-close class="rechargeDialog">
			<div class="centerPayInpView">
				<el-input class="pay_inp" value="￥199/年" readonly></el-input>
			</div>
			<div class="centerPayList">
				<div class="centerPayView">
					<el-radio v-model="payType" label="1"><img src="@/assets/pay/weixin.png" alt>微信支付</el-radio>
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button type="primary" @click="vipSave">
						购买
					</el-button>
				</span>
			</template>
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			context?.$toolUtil.storageSet("vip", res.data.data.vip)
	...
	const vipClick = () => {
		if(userForm.value.vip=='是'){
			context?.$toolUtil.message('您已是我们尊贵的会员','success')
			return false
		}
		payType.value = ''
		payType1.value = ''
	...
	const vipSave = () => {
		...
		context?.$http({
			...
			context?.$toolUtil.message('会员购买成功','success',()=>{
